...
import {
  ...
  getPhoneCameraBridgeSetupBaseUrl,
  getPhoneCameraCaFingerprint,
...
const PHONE_CAMERA_STATE_CHANGED_CHANNEL =
  "recordly-phone-camera:state-changed";
const PHONE_CAMERA_FRAME_TIMEOUT_MS = 5_000;
const PHONE_CAMERA_PAIRING_TTL_MS = 10 * 60_000;
...
function scheduleFrameLivenessCheck(): void {
  ...
  frameLivenessTimer = setTimeout(() => {
    ...
    setPhoneCameraState({
      ...
      status: "pending",
      message:
        "Phone camera stopped sending frames. Waiting for it to reconnect.",
      error: undefined,
    });
...
function setPhoneCameraState(
  patch: Partial<PhoneCameraState>,
): PhoneCameraState {
...
configurePhoneCameraBridgeSession({
  getSession: () => ({
    ...
    pairingToken: phoneCameraState.pairingToken,
    pairingExpiresAtMs: phoneCameraState.pairingExpiresAtMs,
  }),
  onConnect: ({ sessionId, pairingToken, remoteAddress }) => {
    if (
      phoneCameraState.sessionId !== sessionId ||
      phoneCameraState.pairingToken !== pairingToken ||
      (phoneCameraState.pairingExpiresAtMs ?? 0) <= Date.now()
    ) {
      return false;
    }
  ...
  onFrame: ({
    ...
    if (
      !/^data:image\/(jpeg|jpg|png);base64,[A-Za-z0-9+/=]+$/i.test(frameDataUrl)
    ) {
      return false;
    }
...
function buildPairingUrl(
  ...
  return `${baseUrl}/phone-camera-setup?session=${encodeURIComponent(sessionId)}&token=${encodeURIComponent(pairingToken)}`;
}

export function restorePhoneCameraPreviewIfConnected(): void {
  if (
    phoneCameraPreviewEnabled &&
    phoneCameraState.active &&
    phoneCameraState.connected
  ) {
    showPhoneCameraOverlayWindow();
  }
}
...
export function registerPhoneCameraHandlers() {
  ...
  ipcMain.handle(
    "recordly-phone-camera:start",
    async (_event, options?: { reason?: string }) => {
      phoneCameraPreviewEnabled = true;
      if (
        phoneCameraState.active &&
        phoneCameraState.sessionId &&
        phoneCameraState.pairingToken &&
        (phoneCameraState.pairingExpiresAtMs ?? 0) > Date.now()
      ) {
        if (phoneCameraPreviewEnabled) {
          showPhoneCameraOverlayWindow();
      ...
      const pairingToken = buildPairingToken();
      const pairingExpiresAtMs = Date.now() + PHONE_CAMERA_PAIRING_TTL_MS;
      setPhoneCameraState({
        active: true,
        connected: false,
        status: "pending",
      ...
      try {
        await ensurePhoneCameraBridgeServer();
        const setupBaseUrl = getPhoneCameraBridgeSetupBaseUrl();
        if (!setupBaseUrl) {
          throw new Error("Phone camera certificate setup server is unavailable.");
        }
        const state = setPhoneCameraState({
          ...
          caFingerprint: getPhoneCameraCaFingerprint() ?? undefined,
        });
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Unable to start phone camera pairing.";
        const state = setPhoneCameraState({
          active: false,
          status: "error",
          message,
  ...
  ipcMain.handle(
    "recordly-phone-camera:prepare-recording-preview",
    async () => {
  ...
  ipcMain.handle("recordly-phone-camera:forget", async () => {
    ...
    destroyPhoneCameraOverlayWindow();
    const state = setPhoneCameraState(
      createInactivePhoneCameraState(phoneCameraState),
    );
    return { success: true, ...state };
  });
