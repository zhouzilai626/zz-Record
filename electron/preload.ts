...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	getPhoneCameraState: () => ipcRenderer.invoke("recordly-phone-camera:get-state"),
	phoneCameraStart: (options?: { reason?: string }) =>
		ipcRenderer.invoke("recordly-phone-camera:start", options),
	...
	onPhoneCameraStateChanged: (
		...
		const listener = (
			_event: Electron.IpcRendererEvent,
			state: import("../src/lib/phoneCamera").PhoneCameraState,
		) => callback(state);
		ipcRenderer.on("recordly-phone-camera:state-changed", listener);
		return () => ipcRenderer.removeListener("recordly-phone-camera:state-changed", listener);
	},
