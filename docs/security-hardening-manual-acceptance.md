...
| 编号 | 操作 | 期望结果 | 结果 |
| --- | --- | --- | --- |
| CA-01 | 使用首次连接的手机完成 CA 安装和 HTTPS 访问。 | 安装步骤可完成，HTTPS 配对页可打开，摄像头授权可用。 | ☑ 通过（用户于 2026-07-18 确认） |
| CA-02 | 在非受信任公共网络或无法确认网络完整性时尝试配对。 | 不继续安装 CA；记录为不支持的高风险环境。 | ☑ 通过（用户于 2026-07-18 确认） |
| CA-03 | 记录 CA 下载入口是否无须当前会话即可访问。 | 若仍可裸访问，标记为后续 `phone-camera-trust-bootstrap` 工作项，不能宣称已解决。 | ☑ 通过（用户于 2026-07-18 确认；裸访问结果未单独记录） |
