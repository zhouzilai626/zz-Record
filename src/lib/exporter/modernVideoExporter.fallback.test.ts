...
describe("ModernVideoExporter native fallback routing", () => {
	...
	it("skips Windows auto static-layout and stays on the native streaming route when webcam reacts to zoom", async () => {
		vi.stubGlobal("navigator", {
			platform: "Win32",
			userAgent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
		});
		...
		const exporter = new ModernVideoExporter({
			...
			experimentalNvidiaCudaExport: true,
			backendPreference: "auto",
			zoomRegions: [{ id: "zoom-1", startMs: 2_000, endMs: 6_000, x: 0.5, y: 0.5, zoom: 2 }],
			webcam: {
				enabled: true,
