...
describe("ModernVideoExporter native static-layout eligibility", () => {
	...
	it("skips native static layout when webcam react-to-zoom is enabled on zoomed projects", () => {
		const exporter = createExporter({
			zoomRegions: [
				{
					id: "zoom-1",
		...
		).toBe("unsupported-webcam-react-to-zoom");
	});

	it("allows native static layout when webcam react-to-zoom is disabled", () => {
		const exporter = createExporter({
			zoomRegions: [
				{
					id: "zoom-1",
