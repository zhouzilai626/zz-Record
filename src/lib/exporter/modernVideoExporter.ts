...
export class ModernVideoExporter {
	...
	private hasUnsupportedNativeStaticLayoutWebcamReactToZoom(): boolean {
		const webcam = this.config.webcam;
		if (!webcam?.enabled) {
			return false;
		}
		...
		return webcam.reactToZoom !== false;
	}
	...
	private getNativeStaticLayoutSkipReasons(
		...
		if (this.hasUnsupportedNativeStaticLayoutWebcamReactToZoom()) {
			reasons.push("unsupported-webcam-react-to-zoom");
		}
