	...
	return (
		window as unknown as {
			electronAPI: {
				...
				getLocalMediaUrl: ReturnType<typeof vi.fn>;
			};
		}
	).electronAPI;
}
...
describe("StreamingVideoDecoder local media loading", () => {
	...
	it("loads loopback media-server URLs directly into WebDemuxer", async () => {
		...
		expect(getElectronApi().readLocalFile).not.toHaveBeenCalled();
	...
	it("normalizes absolute local paths to file URLs before loading them", async () => {
		...
		expect(getElectronApi().getLocalMediaUrl).toHaveBeenCalledWith("/tmp/capture.mp4");
		expect(mockDemuxerLoad).toHaveBeenCalledWith(
	...
	it("falls back to a readable File when direct loading fails", async () => {
		...
		getElectronApi().readLocalFile = vi.fn(async () => ({
			success: true,
		...
		expect(getElectronApi().readLocalFile).toHaveBeenCalledWith("/tmp/fallback.mp4");
	});
});
