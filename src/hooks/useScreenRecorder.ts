...
async function loadPhoneCameraFrameImage(frameDataUrl: string): Promise<HTMLImageElement> {
...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const phoneCameraCanvas = useRef<HTMLCanvasElement | null>(null);
	const phoneCameraPollTimeout = useRef<number | null>(null);
	const phoneCameraFrameRequestInFlight = useRef(false);
	const phoneCameraPaused = useRef(false);
	...
	const phoneCameraSessionActive = useRef(false);
	...
	const stopWebcamRecorder = useCallback(async () => {
		...
		phoneCameraFrameRequestInFlight.current = false;
		phoneCameraPaused.current = false;
	...
	const prepareWebcamRecorder = useCallback(async () => {
		...
		if (isPhoneCameraDeviceId(webcamDeviceId)) {
			resolvedWebcamPath.current = null;
			...
			try {
				...
				if (!context) {
					throw new Error("Failed to create phone camera recording canvas");
				}

				phoneCameraCanvas.current = canvas;
				phoneCameraPaused.current = false;
				const captureStream = canvas.captureStream(PHONE_CAMERA_RECORDING_FRAME_RATE);
				webcamStream.current = captureStream;
				...
				recorder.onstop = async () => {
					...
					try {
						...
						const storeResult = await window.electronAPI.storeRecordedVideo(
							arrayBuffer,
							webcamFileName,
						);
				...
				const pumpPhoneCameraFrame = async () => {
					if (!phoneCameraSessionActive.current || phoneCameraPaused.current) {
						phoneCameraPollTimeout.current = window.setTimeout(
							() => void pumpPhoneCameraFrame(),
							PHONE_CAMERA_RECORDING_POLL_MS,
						);
						return;
					}
					if (phoneCameraFrameRequestInFlight.current) {
						phoneCameraPollTimeout.current = window.setTimeout(
							() => void pumpPhoneCameraFrame(),
							PHONE_CAMERA_RECORDING_POLL_MS,
						);
					...
					phoneCameraFrameRequestInFlight.current = true;
					try {
						const frame = await window.electronAPI.phoneCameraGetFrame();
						if (frame.success && frame.frameDataUrl && phoneCameraCanvas.current) {
							const image = await loadPhoneCameraFrameImage(frame.frameDataUrl);
							const width =
							...
							if (width > 0 && height > 0 && phoneCameraCanvas.current) {
								if (
									...
									phoneCameraCanvas.current.height !== height
								) {
									phoneCameraCanvas.current.width = width;
									phoneCameraCanvas.current.height = height;
								}
								context.clearRect(
									0,
									0,
									phoneCameraCanvas.current.width,
									phoneCameraCanvas.current.height,
								);
								context.drawImage(
									image,
									0,
						...
						console.warn("Failed to capture phone camera frame for recording:", error);
					} finally {
						phoneCameraFrameRequestInFlight.current = false;
						if (phoneCameraSessionActive.current) {
							phoneCameraPollTimeout.current = window.setTimeout(
	...
	const beginWebcamCapture = useCallback(() => {
		if (phoneCameraSessionActive.current) {
			...
			if (webcamRecorder.current?.state === "inactive") {
				webcamRecorder.current.start(RECORDER_TIMESLICE_MS);
			}
			return;
		}
		const recorder = webcamRecorder.current;
	...
	const pauseRecording = useCallback(() => {
		...
		if (nativeScreenRecording.current) {
			void (async () => {
				...
				if (phoneCameraSessionActive.current) {
					phoneCameraPaused.current = true;
				}
	...
	const cancelRecording = useCallback(() => {
		...
		if (phoneCameraSessionActive.current) {
			phoneCameraSessionActive.current = false;
			void window.electronAPI.phoneCameraStop().catch((error) => {
				console.warn("Failed to stop phone camera session:", error);
			});
		}
